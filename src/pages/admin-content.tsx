...
export default function ContentManagementPage() {
  ...
  useEffect(() => {
    ...
    setEducation([...config.education].sort((a, b) => a.localeCompare(b)));
                        ...
                        <CollapsibleContent className="pt-2 pl-4">
                          <div className="flex flex-wrap gap-2 mb-3">
                            {[...cityList].sort((a, b) => a.localeCompare(b)).map(city => (
                              <Badge key={city} variant="secondary" className="text-sm py-0.5 px-3 flex items-center gap-2 border bg-background shadow-sm">
                                {city}
                                <button onClick={() => {
